...
const server = http.createServer((req, res) => {
  ...
  if (urlparse.pathname === "/todo" && req.method === "OPTIONS") {
    sendResponse(res, 200, true);
  } else if (urlparse.pathname === "/todo" && req.method === "GET") {
  ...
  } else if (urlparse.pathname === "/todo" && req.method === "POST") {
    req.on("data", (data) => {
        ...
        if (id) {
          ...
          if (!title) {
            sendResponse(res, 400, {
              message: "no title found in body request!",
            });
          } else {
  ...
  } else if (urlparse.pathname === "/todo" && req.method === "DELETE") {
    const search = urlparse.search;
